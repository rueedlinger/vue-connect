...
def test_api_not_allowed(client):
    resp = client.post('/api/status', data=json.dumps({}),  content_type='application/json')
...
def test_api_post_with_data(client):
    resp = client.post('/api/connectors/foo/config', data=json.dumps({}),  content_type='application/json')
    assertNotReachable(resp)

    resp = client.post('/api/connectors', data=json.dumps({'name': 'foo'}),  content_type='application/json')
    assertNotReachable(resp)

    resp = client.post('/api/connectors', data=json.dumps({}),  content_type='application/json')
